...
L.tripgoRouting.mapLayer = (function () {
  ...
  function popUpSelector(latlng) {
    return (
      ...
      'onclick=\'L.tripgoRouting.mapLayer.createMarker("from",' +
      latlng.lat +
      "," +
      latlng.lng +
      ")'>" +
      ...
      "</div>" +
      " <div class='addressSelector' onclick='L.tripgoRouting.mapLayer.createMarker(\"to\", " +
      latlng.lat +
      "," +
      latlng.lng +
      ")'>" +
  ...
  function checkOptions(options) {
    if (options === undefined) options = defaultOptions;
      ...
      if (
        !L.tripgoRouting.has(options, "googleTile") ||
        options.googleTile === undefined
      )
        options.gogoleTile = defaultOptions.googleTile;
      if (
        !L.tripgoRouting.has(options, "mapCenter") ||
        options.mapCenter === undefined
      )
      ...
      if (
        !L.tripgoRouting.has(options, "tripgoApiKey") ||
        options.tripgoApiKey.includes("TripGo")
      )
  ...
  return {
    initialize: function (options) {
      ...
      options.googleTile === undefined || options.googleTile
        ? setGoogleTile()
        : setOSMTile();
      ...
      if (floatPanel) tripsPanel.className = "selectorFloatPanel";
      else tripsPanel.className = "selectorPanel";

      document.body.insertBefore(tripsPanel, L.DomUtil.get(mapId));

      L.DomEvent.on(window, "resize", function () {
        L.tripgoRouting.mapLayer.mapResize(
          window.innerWidth - L.tripgoRouting.tripWidget.getWidth(),
          window.innerHeight
        );
        L.tripgoRouting.tripWidget.getWidget().style.height =
...
L.tripgoRouting.routeService = (function () {
  let templatesCache = [];
  const transportModes = ["pt_pub", "wa_wal"];
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (L.tripgoRouting.has(result, "groups")) {
          templatesCache = L.tripgoRouting.util.parseTemplates(
            result.segmentTemplates
          );
      ...
      error: function (data) {
        ...
        L.tripgoRouting.mapLayer.getMessenger().hideMessage();
        L.tripgoRouting.mapLayer
          .getMessenger()
          .error(
            "service-not-available, The routing service is currently not available, <br> please check your API key or try again later"
          );
      },
    });
  }
  ...
  return {
    ...
    route: function (tripgoApiKey, from, to) {
      if (
        L.tripgoRouting.validLatLng(from) &&
        L.tripgoRouting.validLatLng(to)
      ) {
        L.tripgoRouting.mapLayer
          .getMessenger()
          .info("getting routes form BebraClub server ...");
        let multimodal = "";
...
class Segment {
  constructor(jsonSegment) {
    ...
    if (
      jsonSegment.serviceColor !== null &&
      jsonSegment.serviceColor !== undefined
    ) {
  ...
  get getDistanceString() {
    if (this.meters !== undefined) {
      if (this.meters < 1000) return this.meters + " m";
      else return (this.meters / 1000).toFixed(1) + " km";
    } else return "";
  }
}
      ...
      for (let i = 0; i < this.segments.length; i++) {
        if (this.segments[i].waypoints !== undefined) {
          ...
          for (let j = 0; j < this.segments[i].waypoints.length; j++) {
            ...
            if (this.bounds === null) this.bounds = polyline.getBounds();
            else this.bounds.extend(polyline.getBounds());
...
L.tripgoRouting.tripWidget = (function () {
  ...
  function createTripLine(trip, id) {
    ...
    L.DomEvent.on(tripline, "mouseover", function () {
      ...
      if (L.tripgoRouting.mapLayer.getTripDisplaying() !== undefined)
        L.tripgoRouting.mapLayer
          .getTripDisplaying()
          .removeFromMap(L.tripgoRouting.mapLayer.getMap());
  ...
  function segmentDetailsWidget(segment) {
    ...
    if (L.tripgoRouting.has(segment.modeInfo, "remoteIcon")) {
      htmlIcon = img(
        L.tripgoRouting.util.getTransportIconSVG(
          segment.modeInfo.remoteIcon,
          true
        ),
        "icon"
      );
      ...
      let path = L.tripgoRouting.util.getTransportIconSVG(
        segment.modeInfo.localIcon,
        false
      );
      if (path !== undefined) {
    ...
    if (segment.modeIdentifier !== undefined) {
      ...
      if (segment.modeIdentifier === "pt_pub")
        text.innerHTML =
          ...
          "</span>" +
          "<br>" +
          L.tripgoRouting.util.getTime(segment.startTime);
      else {
  ...
  return {
    initialize: function () {
      if (!this.isVisible()) {
        ...
        L.tripgoRouting.mapLayer.mapResize(
          window.innerWidth - this.getWidth(),
          L.tripgoRouting.mapLayer.height
        );
      }
    },
    ...
    getWidth: function () {
      if (this.isVisible() && !L.tripgoRouting.mapLayer.selectorPanelIsFloat())
        return 400;
      else return 0;
    },
...
L.tripgoRouting.util = (function () {
  ...
  return {
    ...
    createIcon: function (mode) {
      ...
      else icon.src = this.getTransportIconSVG(mode.localIcon, false);
    ...
    getTransportIconSVG: function (modeCode, remote) {
      if (remote) return remoteIconUrl + remoteIconString(modeCode);
      else {
        if (
          modeCode !== "wait" &&
          modeCode !== "transfer" &&
